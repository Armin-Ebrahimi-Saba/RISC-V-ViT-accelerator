...
module student_gemm_soc_tb;
  ...
  // Same topology as student.sv: the accelerator's host port goes through a
  // real tlul_socket_m1 before it reaches memory. The socket's own assertions
  // (DataKnown_A in its FIFOs) are the point of this testbench -- they are the
  // ones that fired in the system simulation.

  tlul_pkg::tl_h2d_t host_h2d [2];
  tlul_pkg::tl_d2h_t host_d2h [2];
  tlul_pkg::tl_h2d_t sock_h2d;
  tlul_pkg::tl_d2h_t sock_d2h;

  // Port 0 idle, driven exactly as student_dma drives it out of reset.
  assign host_h2d[0] = '{
  ...
  assign host_h2d[1] = mem_h2d;
  assign mem_d2h     = host_d2h[1];
  ...
  initial begin
    ...
    $display("=======================================");
    if (errors == 0)
      $display("student_gemm_soc_tb PASSED (%0d words checked)", checks);
    else
      $display("student_gemm_soc_tb FAILED (%0d errors, %0d words checked)", errors, checks);
    $display("=======================================");
    $finish;
  end
