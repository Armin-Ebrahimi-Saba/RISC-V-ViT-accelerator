...
module student_gemm_ddr_tb;
  ...
  student_gemm #(
      ...
      // Match the board: student.sv instantiates the DDR3-facing block with
      // MAX_INFLIGHT(1).
      .MAX_INFLIGHT(1)
  ) dut (
      .clk_i     (clk),
  ...
  tlul_test_mem #(
      .SIZE_WORDS(262144),
      .BASE_ADDR (MEM_BASE),
      // DDR3-like: accepts far fewer requests in flight than the block can
      // issue, and answers slowly. The 8-slot model never exercised the case
      // where a_ready is deasserted for long stretches.
      .DEPTH     (1),
      .MIN_LAT   (8),
      .MAX_LAT   (40)
  ) memory (
  ...
  initial begin
    ...
    // The exact shape that hung on the board was N=81 K=588 M=384; M is
    // reduced here only to keep simulation time sane, since the block stalled
    // on its very first job.
    // The board hangs on the LAST tile of N=81, which is a single row
    // (81 = 5*16 + 1). Isolate that: a lone nt=1 tile, then the 16->1
    // transition, then the 16-row case for contrast.
    run_gemm(1,  588, 4);    // single-row tile on its own
    run_gemm(17, 588, 4);    // 16-row tile followed by a 1-row tile
    run_gemm(16, 588, 4);    // full tile, for contrast
    ...
    $display("=======================================");
    if (errors == 0)
      $display("student_gemm_ddr_tb PASSED (%0d words checked)", checks);
    else
      $display("student_gemm_ddr_tb FAILED (%0d errors, %0d words checked)", errors, checks);
    $display("=======================================");
    $finish;
  end
