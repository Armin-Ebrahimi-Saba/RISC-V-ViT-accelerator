...
| `dav2_blob_int.py` | **Removes the floats from the blob.** Converts row scales and biases to (m, sh) pairs, LayerNorm γ and β to Q15 and Q16, and the embeddings to Q24 (blob version 2 → 3). With `--phi-header` it writes `dav2_gelu_phi.h`. Needs only `numpy`. |
