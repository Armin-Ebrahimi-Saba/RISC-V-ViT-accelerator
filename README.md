...
| `dav2_xf.h` | **Real numbers without floating point.** A scale is stored as two integers, value = m · 2^−sh. Multiply, add, divide, 1/x and 1/√x on these pairs use only integer instructions. The program has no float operation at all. |
| `dav2_gelu_phi.h` | Generated table of the normal distribution function Φ (Q30), from which the GELU table is built. |
...
| File | What it does |
|---|---|
| `export_dav2.py` | **Makes the weight blob.** Loads the PyTorch model, quantises every weight to int8 with a per-row scale, and writes it all into one file with a directory. It then converts the file to integer parameters with `dav2_blob_int.py`. Needs `torch`, `numpy`, `pillow`. |
| `dav2_blob_int.py` | **Removes the floats from the blob.** Folds the final LayerNorm's γ and β into proj0..3 (from the checkpoint), converts row scales and biases to (m, sh) pairs, the block LayerNorms' γ and β to Q15 and Q16, and the embeddings to Q24 (blob version 2 → 4). With `--phi-header` it writes `dav2_gelu_phi.h`. Needs `numpy` and `torch`. |
| `cyclemodel/` | **Cycle estimates without the board.** Runs the engine's RISC-V build in the Unicorn CPU emulator with a CV32E40P timing model calibrated against the board's boot microbenchmark. Kernels on synthetic data, source-line profiles, or a whole frame (accelerator jobs take no time). Needs `pip install unicorn pyelftools`. |
