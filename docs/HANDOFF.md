...
**Rounds eight to eighteen are not measured on the board yet.** The board was
not connected. Their changes are verified on the PC (host build,
accelerator emulator, 11 test images) and estimated with
`tools/cyclemodel/` (`PERFORMANCE.md` §6): about 2.22 s per frame
...
self-test ok" (round seventeen) and "row-sums self-test ok" (round
eighteen), and the accelerator report about 1303 jobs per frame.
