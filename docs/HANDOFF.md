...
**Rounds eight to seventeen are not measured on the board yet.** The board was
not connected. Their changes are verified on the PC (host build,
accelerator emulator, 11 test images) and estimated with
`tools/cyclemodel/` (`PERFORMANCE.md` §6): about 2.35 s per frame
...
(round fourteen), "result-RAM self-test ok" (round sixteen) and "tap-reuse
self-test ok" (round seventeen), and the accelerator report about 1302
jobs per frame.
